...
One common practice is to devote an area of the screen to a virtual gamepad like interface. This can be seen in many games and may work for some titles but I want to argue here that this is a bad solution even if it is straight forward. That type of solution wastes a good part of the screen and unlike its real world counterparts (a.k.a. console controllers) virtual gamepads provide really bad tactile feedback.
...
![Craigen screen, tip: the player will fall on that hole](images/originals/craigen-1.png)
...
The main thing about gamepad controllers is that sometimes they are really different from one another regarding button placement. If you're supporting gamepads, you should offer a screen for the user to alter controller configuration such as what button is pressed to fire or jump. As an anecdote let me tell you that I have a cheap chinese gamepad that was soldered wrong (I believe it was a mistake) and has the directional axis inverted. I can't use that controller with games that do not allow me to customize the controls due to it being kinda upside down internally.

## Controlling Our Game

For the sake of brevity we're only going to implement touch and keyboard input. Phaser has support for gamepads out of the box but this is out of the scope of the simple game we're building. If you want to know more about gamepad APIs in Phaser refer to the [Gamepad documentation](http://docs.phaser.io/Phaser.Gamepad.html).

What are the needs of Foxnoid regarding player control? Basically the player controls a paddle which moves left or right and thats it. Like that Craigen game we're doing to divide the screen in two halves and touching any half will cause the paddle to start moving in that direction. We're going to add support for keyboard input with the arrow keys.
